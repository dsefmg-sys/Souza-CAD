...
export default function RequerimentoModal({ open, onOpenChange, imovel, onChangeImovel, tecnico, areaRealHa, requerente, transmitente, tipoAto, tiposAtos, partesAdicionais, onChangePessoas, sugProp, correcoes, onBaixar, onBaixarPacoteZip, glebas, glebaAtivaId, onSalvarProjeto, onMinimizar }: Props & { onMinimizar?: () => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(val) => { if (!val) handleCloseRequest(); else onOpenChange(val); }}>
      <DialogContent onMinimize={onMinimizar} className="w-[96vw] sm:w-[92vw] max-w-[1240px] max-h-[92vh] sm:max-h-[88vh] flex flex-col p-3 sm:p-5 overflow-hidden shadow-2xl" onEscapeKeyDown={(e) => e.preventDefault()} onInteractOutside={(e) => e.preventDefault()}>
