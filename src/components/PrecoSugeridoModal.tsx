...
export default function PrecoSugeridoModal({ open, onOpenChange, areaHa = 0 }: Props) {
  const [aba, setAba] = useState<'imovel' | 'tabela'>('imovel');
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        ...
        {/* Fator base */}
        <div className="flex items-end gap-3 rounded border bg-muted/30 p-2.5">
          <label className="flex flex-col gap-1 text-xs">
            <span className="font-semibold text-muted-foreground">Valor base (fator)</span>
            <input
              className="w-28 rounded border bg-background px-2 py-1.5 text-sm font-mono"
              value={fatorStr}
              inputMode="numeric"
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 rounded border border-emerald-200 bg-emerald-50/60 p-2.5 dark:border-emerald-900 dark:bg-emerald-950/30">
          ...
          <p className="min-w-0 flex-1 text-[10px] leading-tight text-muted-foreground">
