...
// Extrai dados do IMÓVEL/PROPRIETÁRIO de um texto colado (matrícula, escritura, etc.) usando o
// Gemini. Roda NO SERVIDOR: a chave (GOOGLE_GENAI_API_KEY) nunca vai para o navegador.
...
export async function POST(req: Request) {
  ...
  let texto = '';
  try { ({ texto } = await req.json()); } catch { /* corpo inválido */ }
  if (!texto || texto.trim().length < 10) return NextResponse.json({ erro: 'Cole um texto maior para a IA extrair.' }, { status: 400 });
  ...
  const instrucao =
    ...
    'cpfConjugeProprietario, municipio, comarca, areaAnteriorHa. Não invente dados que não estejam no texto.\n\n' +
    'TEXTO:\n"""' + texto.slice(0, 12000) + '"""';
